...
4. Khi được hỏi, điền các biến **Secret** (sync: false):
   ...
   - `CLOUDINARY_*`, `AI_API_KEY`, `MAILJET_*` (gửi mail trên Render) hoặc `SMTP_*`
...
### Mailjet (khuyến nghị bổ sung trên Render)

1. [mailjet.com](https://www.mailjet.com/) → **REST API** → API Key + Secret Key.
2. **Senders & Domains** → xác minh email gửi.
3. Trên Render: `MAILJET_API_KEY`, `MAILJET_SECRET_KEY`, `MAILJET_SENDER_EMAIL`, `MAILJET_SENDER_NAME` (tùy chọn).
